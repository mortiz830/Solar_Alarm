...
class TimezoneRepository(application: Application?) : RepositoryBase() {
    private val timezoneDao: TimezoneDao?
    val all: LiveData<List<Timezone?>?>?
    ...
    fun Insert(timezone: Timezone?) {
        SolarAlarmDatabase.Companion.databaseWriteExecutor.execute(Runnable { timezoneDao!!.Insert(timezone) })
    }

    fun Update(timezone: Timezone?) {
        SolarAlarmDatabase.Companion.databaseWriteExecutor.execute(Runnable { timezoneDao!!.Update(timezone) })
    }

    fun delete(timezone: Timezone?) {
        SolarAlarmDatabase.Companion.databaseWriteExecutor.execute(Runnable { timezoneDao!!.delete(timezone) })
    }
